...
class ProductController:    
    ...
    def update_product_dynamic(self, product, condition):
        ...
        while not confirm:
            ...
            if prompt_confirm == 's':
                ...
                addons_view.clear_screen()
                addons_view.show_title_menu(f'ACTUALIZAR')
                break                            
            else:
    ...
    def dynamic_product_controller(self, title, condition):
        ...
        while not back:
            invalid = False
            not_found = False
            prompt = input(f'\t Ingrese el {condition} del producto a {title}: ').strip()           
    ...
    def low_stock_report_controller(self):
        ...
        if products:        
            product_view.show_products(products, 'REPORTE de PRODUCTOS con STOCK BAJO')
        else:
